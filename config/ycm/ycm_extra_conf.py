...
def FlagsForCompilationDatabase(root, filename):
    try:
        compilation_db_path = FindByPath(root,'compile_commands.json')
        ...
        s = " "
        logging.info(list(compilation_info.compiler_flags_))
        logging.info(compilation_info.compiler_working_dir_)
        if not compilation_info:
            logging.info("will try from aliroot.cxxx")
            compilation_info = GetCompilationInfoForFile(compilation_db,"aliroot.cxx")
            if not compilation_info:
